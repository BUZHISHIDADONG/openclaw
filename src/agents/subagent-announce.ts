...
type SubagentOutputSnapshot = {
  ...
  silentAfterVisibleProgress: boolean;
};
...
function summarizeSubagentOutputHistory(messages: Array<unknown>): SubagentOutputSnapshot {
  const snapshot: SubagentOutputSnapshot = {
    assistantFragments: [],
    toolCallCount: 0,
    silentAfterVisibleProgress: false,
  };
  for (const message of messages) {
    ...
    if (role === "assistant") {
      ...
      if (isAnnounceSkip(text) || isSilentReplyText(text, SILENT_REPLY_TOKEN)) {
        if (
          snapshot.assistantFragments.length > 0 ||
          Boolean(snapshot.latestAssistantText) ||
          Boolean(snapshot.latestRawText)
        ) {
          snapshot.silentAfterVisibleProgress = true;
        }
      ...
      if (!snapshot.silentAfterVisibleProgress) {
...
function selectSubagentOutputText(
  ...
  if (
    ...
    snapshot.silentAfterVisibleProgress &&
    snapshot.latestSilentText
  ) {
    return snapshot.latestSilentText;
  }
  if (snapshot.latestSilentText && !snapshot.latestAssistantText) {
    return snapshot.latestSilentText;
  }
...
async function readSubagentOutput(
  ...
  try {
    latestAssistant = await agentStep.readLatestAssistantReply({
      sessionKey,
      limit: 100,
    });
  } catch {
    // Best-effort only; fall back to full history parsing below.
  }
  ...
  const historySelected = selectSubagentOutputText(snapshot, outcome);
  if (outcome?.status === "timeout") {
    return historySelected ?? latestAssistant;
  }
  return latestAssistant?.trim() ? latestAssistant : historySelected;
}
...
async function maybeQueueSubagentAnnounce(params: {
  ...
  const isActive = piEmbedded.isEmbeddedPiRunActive(sessionId);

  const shouldSteer = queueSettings.mode === "steer" || queueSettings.mode === "steer-backlog";
  if (shouldSteer) {
    const steered = piEmbedded.queueEmbeddedPiMessage(sessionId, params.steerMessage);
    if (steered) {
      return "steered";
    }
  }
...
export async function runSubagentAnnounceFlow(params: {
  ...
  try {
    ...
    if (childSessionId && piEmbedded.isEmbeddedPiRunActive(childSessionId)) {
      const settled = await piEmbedded.waitForEmbeddedPiRunEnd(childSessionId, settleTimeoutMs);
      if (!settled && piEmbedded.isEmbeddedPiRunActive(childSessionId)) {
    ...
    if (!childCompletionFindings) {
      ...
      if (
        ...
        !reply?.trim() &&
        childSessionId &&
        piEmbedded.isEmbeddedPiRunActive(childSessionId)
      ) {
        shouldDeleteChildSession = false;
        return false;
      }
