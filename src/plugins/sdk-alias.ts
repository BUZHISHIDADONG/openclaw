...
export function resolvePluginSdkAliasCandidateOrder(params: {
  ...
  const isSourceExtensionEntry =
    normalizedModulePath.includes("/extensions/") && !normalizedModulePath.includes("/dist/");
  // Source extension entries still work against built plugin-sdk subpaths, and
  // preferring dist avoids pulling the entire source plugin-sdk graph through
  // Jiti during gateway/plugin bootstrap.
  return isDistRuntime || params.isProduction || isSourceExtensionEntry
    ? ["dist", "src"]
    : ["src", "dist"];
}
