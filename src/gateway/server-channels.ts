...
type ChannelManagerOptions = {
  ...
  resolvePluginRuntime?: () => PluginRuntime;
};
...
export function createChannelManager(opts: ChannelManagerOptions): ChannelManager {
  ...
  const getPluginRuntime = (): PluginRuntime | undefined => {
    return pluginRuntime ?? resolvePluginRuntime?.();
  };
    ...
    await Promise.all(
      accountIds.map(async (id) => {
        ...
        try {
          ...
          const resolvedPluginRuntime = getPluginRuntime();
          const task = startAccount({
            ...
            ...(resolvedPluginRuntime ? { pluginRuntime: resolvedPluginRuntime } : {}),
          });
